...
cycleThroughList([Head|_], Head).

cycleThroughList([_|Tail], Result) :-
    cycleThroughList(Tail, Result).

interpret(_, _, true) :- !.

interpret(Bound, List, (E1, E2)) :-
    !,
    interpret(Bound, List, E1),
    interpret(Bound, List, E2).

interpret(Bound, List, Call) :-
    functor(Call, Name, Arity),
    (cycleThroughList(List, /(Name, Arity)) ->
        (decBound(Bound, NewBound),
        clause(Call, Body),
        interpret(NewBound, List, Body));
        (clause(Call, Body),
        interpret(Bound, List, Body))).
