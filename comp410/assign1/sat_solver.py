...
def solve(goals, literals):
    if isinstance(goals, Nil):
        ...
        if isinstance(goals.head, Literal):
            ...
            left_hand_side = solve(Cons(goals.head.left, goals.tail), literals)
            if left_hand_side is not None:
                return left_hand_side
            else:
                return solve(Cons(goals.head.right, goals.tail), literals)
        else:
            raise Exception("Unknown binary operation type: " + goals.__str__())
    else:
        raise Exception("Unknown list type: " + goals.__str__())
...
# my tests that should be satisfiable
# my_sat_tests = [
#                 Literal("x", True),  # x
#
#                 Literal("x", False),  # !x
#
#                 And(And(Literal("x", True),
#                         Literal("y", False)),
#                     Literal("z", False)),  # ((x && !y) && !z)
#
#                 And(And(And(Literal("w", True),
#                             Literal("x", False)),
#                         Literal("y", False)),
#                     Literal("z", True)),  # (((w && x) && !y) && !z)
#
#                 And(Or(And(Literal("w", True),
#                            Literal("x", False)),
#                        Literal("y", False)),
#                     Literal("z", True)),  # (((w && x) || !y) && !z)
#
#                 Or(Or(Or(Literal("w", True),
#                          Literal("x", False)),
#                       Literal("y", False)),
#                     Literal("z", True)),  # (((w || x) || !y) || !z)
#
#                 Or(And(Literal("x", True),
#                        Literal("x", False)),
#                    Literal("x", True)),  # ((x && !x) || x)
#
#                 And(Or(Literal("x", True),
#                        Literal("y", True)),
#                     Or(Literal("x", False),
...
# my tests that should be unsatisfiable
# my_unsat_tests = [
#                 Or(And(Literal("x", True),
#                        Literal("x", False)),
#                    And(Literal("y", True),
...
if __name__ == "__main__":
    run_tests()
    # run_tests(sat_tests, unsat_tests)
    # run_tests(my_sat_tests, my_unsat_tests)
